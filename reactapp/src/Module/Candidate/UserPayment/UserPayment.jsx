...
const [pay,setPay]=useState([])
    ...
    if(b){
        ...
        const newCourse = {
          ...
          a[0].allCourseId.push(newCourse);
        ...
        axios.put(`http://localhost:8080/update/${orgUserId}`,  a[0]).then((response) => {
       ...
       const newUser = {
        userId:12,
        allCourseId:[{courseId: orgCoursId}]
       }

        axios.post('http://localhost:8080/save',newUser )
          .then((response) => {
...
setorgUserId(localStorage.getItem('id'));
setorgCoursId(localStorage.getItem('courseid'));
